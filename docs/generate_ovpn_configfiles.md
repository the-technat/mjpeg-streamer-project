...
`docker-compose run --rm openvpn ovpn_genconfig -u udp://vpn.domain.tld -C 'AES-256-CBC' -a 'SHA256'`
...
cause most of these settings are looked up in ovpn_env.sh we make these changes there to, according to what we did above:
