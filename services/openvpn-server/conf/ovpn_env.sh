declare -x OVPN_AUTH=SHA256
declare -x OVPN_CIPHER=AES-256-CBC
declare -x OVPN_CLIENT_TO_CLIENT=
declare -x OVPN_CN=live.technat.ch
declare -x OVPN_COMP_LZO=0
declare -x OVPN_DEFROUTE=1
declare -x OVPN_DEVICE=tun
declare -x OVPN_DEVICEN=0
declare -x OVPN_DISABLE_PUSH_BLOCK_DNS=1
declare -x OVPN_DNS=1
declare -x OVPN_DNS_SERVERS=([0]="208.67.220.220" [1]="208.67.222.222")
declare -x OVPN_ENV=/etc/openvpn/ovpn_env.sh
declare -x OVPN_EXTRA_CLIENT_CONFIG=()
declare -x OVPN_EXTRA_SERVER_CONFIG=()
declare -x OVPN_FRAGMENT=
declare -x OVPN_KEEPALIVE='10 60'
declare -x OVPN_MTU=
declare -x OVPN_NAT=0
declare -x OVPN_PORT=1194
declare -x OVPN_PROTO=udp
declare -x OVPN_PUSH=()
declare -x OVPN_ROUTES=([0]="192.168.240.0/24")
declare -x OVPN_SERVER=192.168.230.0/24
declare -x OVPN_SERVER_URL=udp://live.technat.ch
declare -x OVPN_TLS_CIPHER=
